...
// ============================================================
// 10. زر واتساب – إرسال رسالة مخصصة مع رابط الصورة
// ============================================================
document.addEventListener('DOMContentLoaded', function() {
  ...
  const propertyTitle = document.getElementById('propertyTitle')?.textContent || 'عقار';
  const propertyPrice = document.getElementById('propertyPrice')?.textContent || '';
  const propertyLocation = document.getElementById('propertyLocation')?.textContent || 'دمشق - سوريا';
  const propertyTag = document.getElementById('propertyTag')?.textContent || 'للبيع';
  const mainImage = document.getElementById('mainImage')?.src || window.location.origin + '/img/9.jpg';

  // تفاصيل المواصفات
  const specArea = document.getElementById('specArea')?.textContent || '';
  const specBedrooms = document.getElementById('specBedrooms')?.textContent || '';
  const specBathrooms = document.getElementById('specBathrooms')?.textContent || '';
  const specYear = document.getElementById('specYear')?.textContent || '';

  // بناء رابط الصورة المطلق
  const imageUrl = mainImage.startsWith('http') ? mainImage : window.location.origin + '/' + mainImage;
...
أنا مهتم بهذا العقار:
 ...
 المساحة: ${specArea}
 الغرف: ${specBedrooms}
 الحمامات: ${specBathrooms}
 سنة البناء: ${specYear}

 رابط الصورة: ${imageUrl}

أتمنى الحصول على استشارة مجانية حول هذا العقار. شكراً!`;

  // تشفير الرسالة للرابط
  const encodedMessage = encodeURIComponent(message);

  // رقم واتساب
  const phoneNumber = '963967514352';
